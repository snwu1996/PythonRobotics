...
class DubinsPathPlannerError(Exception):
    pass
...
PLANNER_TABLE = {'lsl': left_straight_left,
                 'rsr': right_straight_right,
                 'lsr': left_straight_right,
                 'rsl': right_straight_left,
                 'rlr': right_left_right,
                 'lrl': left_right_left}
def dubins_path_planning_from_origin(end_x, end_y, end_yaw, curvature, step_size, whitelist_planners):
    dx = end_x
    dy = end_y
    D = math.hypot(dx, dy)
    d = D * curvature
    ...
    try:
        planning_funcs = [PLANNER_TABLE[planner_key] for planner_key in whitelist_planners]
    except KeyError as ke:
        err_str = '\n'.join([f'Invalid whitelist_planners: {whitelist_planners}.',
                             f'Valid planners are {list(PLANNER_TABLE.keys())}'])
        raise DubinsPathPlannerError(err_str)
...
def dubins_path_planning(s_x, s_y, s_yaw, g_x, g_y, g_yaw, curvature,
                         step_size=0.1,
                         whitelist_planners=list(PLANNER_TABLE.keys())):
    """
    Dubins path planner
    ...
    lp_x, lp_y, lp_yaw, modes, lengths = dubins_path_planning_from_origin(
        le_xy[0], le_xy[1], le_yaw, curvature, step_size, whitelist_planners)
